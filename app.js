//  -------------------------------------------------------------
//  application   ClueMesh
//  maintainer    Qingke Zhang
//  description   profile discovery engine and local API
//  license       AGPL-3.0
//  -------------------------------------------------------------
...
const argv = yarg_.usage('Usage: $0 --username "johndoe" --websites "youtube tiktok"\nUsage: $0 "fast" --username "johndoe"')
  ...
  .boolean('extract')
  .describe('metadata', 'Extract public profile metadata if possible')
...
const runtimeDataRoot = process.env.CLUEMESH_DATA_DIR || appRoot
const logsRoot = path.join(runtimeDataRoot, 'logs')
...
export async function startServer ({ host = server_host, port = server_port } = {}) {
  if (active_server) {
    const address = active_server.address()
    return {
      server: active_server,
      host,
      port: address.port,
      url: `http://${host}:${address.port}/app.html`
    }
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    server.once('listening', () => {
      ...
      const url = `http://${host}:${address.port}/app.html`
      console.log('Server started at %s', url)
      resolve({ server, host, port: address.port, url })
    })
  })
}
...
const gui_requested = argv.gui === true || process.argv.includes('--gui')

if (gui_requested && process.env.CLUEMESH_DESKTOP !== '1') {
  startServer().catch(error => {
    console.error('[Error] Could not start the web application:', error.message)
    process.exitCode = 1
  })
} else {
  if (argv.list) {
