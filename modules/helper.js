...
const runtime_data_path = process.env.CLUEMESH_DATA_DIR || path.join(__dirname, '..')
const logs_path = slash(path.join(runtime_data_path, 'logs'))
...
function get_log_file (uuid) {
  ...
  const _string = slash(path.join(logs_path, _uuid + '_log.txt'))
  return _string
}
...
function log_to_file_queue (uuid, msg, table = false, argv = undefined) {
  logs_queue = logs_queue.then(function () {
    return new Promise(function (resolve) {
      const temp_log_file = slash(path.join(logs_path, uuid + '_log.txt'))
